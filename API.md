...
The optional `wallet-avatar` export derives a dense, square Pixel Blast portrait from a public wallet address. The same address produces the same portrait, while different addresses produce different layouts and palettes. Wallet portraits deliberately contain no letter or central core.
